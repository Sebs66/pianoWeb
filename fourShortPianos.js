...
function piano(containerReference) {
    ...
    function printChord() {
        ...
        const keysInfo = {
            ...
            maj7: [0, 4, 7, 11],
            min7: [0, 3, 7, 10],
            dom7: [0, 4, 7, 10], /// Major with minor at the end.
        };
...
function pianoCorto(containerReference) {
    //h1 Defining Selectors.
    ...
    const positionBtn = function (HTMLbutton) {
        /// Get all siblings.
        const Btns = [...HTMLbutton.parentElement.children];
        /// Remove btnSelected from all.
        Btns.forEach((button) => {
            button.classList.remove("btnSelected");
        });
        HTMLbutton.classList.add("btnSelected");
    ...
    function printChord() {
        ...
        switch (chord.position) {
            case "first":
                firstPosition(intervals);
                break;
            case "second":
                secondPosition(intervals);
                break;
            case "third":
    ...
    function setNoteGrafic(chord) {
        const noteGrafic = container.querySelector(".noteGrafic");
        if (!chord.color | !chord.note) {
            noteGrafic.textContent = "Select Chord";
            return;
        }
        let text;
        if (chord.accidental === "natural") {
            text = chord.note + chord.color;
        } else if (chord.accidental === "sharp") {
            text = chord.note + "#" + chord.color;
        } else if (chord.accidental === "bemol") {
            text = chord.note + "b" + chord.color;
        }
        console.log(text);
        noteGrafic.textContent = text;
    }
    function getChordData() {
        /// See which buttons are selected.
        chord.chordSize = [
        ...
        chord.position = [...container.querySelector('.positionButtons').children].filter((button)=>{
            return button.classList.contains("btnSelected")
        })[0].dataset.position;
    ...
    function firstPosition(intervals) {
        ...
        switch (rootNumber) {
            case 0:
                start = 6;
                whiteKeys.slice(start, start + gap).forEach((key) => {
